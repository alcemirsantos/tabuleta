...
package br.ufmg.dcc.tabuleta.decorators;
...
public class LightWeightDecorator extends LabelProvider implements ConcernModelChangeListener, ILightweightLabelDecorator, IPropertyChangeListener
{
	...
	public void decorate( Object pElement, IDecoration pDecoration ) 
	{
		...
		for( String lConcernName : lConcernNames )
		{
			...
			if( lCurrentConcernElements.contains( pElement ))
			{
				...
				boolean lFilter = Tabuleta.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_FILTER_ENABLED );
		...
		//add the decorations
		if( lDoDecorateSuffix && Tabuleta.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SUFFIX_ENABLED ))
		{
			pDecoration.addSuffix( " "+lConcerns.toString() );
		}
		if( lDoDecorateFont && Tabuleta.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_BOLD_ENABLED ))
		{
	...
	private Set<Object> getAllParents( Set<Object> pElements, String pConcern)
	{
		...
		{	
			...
			{
				if ( Tabuleta.getDefault().getConcernModel().getDegree( pConcern, lNext ) <
						Tabuleta.getDefault().getPreferenceStore().getInt( ConcernMapperPreferencePage.P_FILTER_THRESHOLD ))
				{
		...
		if( !Tabuleta.getDefault().getPreferenceStore().getString( ConcernMapperPreferencePage.P_DECORATION_LIMIT ).equals( "" ))
		{
	...
	public static LightWeightDecorator getDecorator()
	{
		IDecoratorManager lDecoratorManager =
			Tabuleta.getDefault().getWorkbench().getDecoratorManager();
		
		if ( lDecoratorManager.getEnabled( "br.ufmg.dcc.tabuleta.decorator" ))
		{
			return (LightWeightDecorator) lDecoratorManager.getBaseLabelProvider( "br.ufmg.dcc.tabuleta.decorator" );
		}
		return null;
	}
