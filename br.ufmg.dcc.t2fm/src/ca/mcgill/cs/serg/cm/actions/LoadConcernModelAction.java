...
public class LoadConcernModelAction implements IObjectActionDelegate, IRunnableWithProgress
{
	...
	public void run( IProgressMonitor pMonitor )
	{
		try
		{
		    ...
		    ConcernMapper.getDefault().setDefaultResource( aFile );
		    ...
		    final int lSkipped = lReader.read( aFile, new LoadMonitor( pMonitor, 
		    		ConcernMapper.getResourceString( "actions.LoadConcernModelAction.TaskName" )) );
		    ...
		    //PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().showView( ConcernMapper.ID_VIEW );
		    if( lSkipped > 0 )
		    {
					...
					public void run()
					{
						...
						for( Shell lNext : lShells ) 
						{
							MessageDialog.openWarning( lNext, 
									ConcernMapper.getResourceString( "actions.LoadConcernModelAction.ErrorDialogTitle" ),
									lSkipped + " " + ConcernMapper.getResourceString( "actions.LoadConcernModelAction.SkippedMessage" ));
							break;
						}
					}});
		    }
		...
		finally
		{
			...
			for( IWorkbenchWindow lWindow : PlatformUI.getWorkbench().getWorkbenchWindows() )
			{
				...
				if( lPage != null )
				{
					IViewPart lView = lPage.findView( ConcernMapper.ID_VIEW );
					if( lView != null )
					{
						((ConcernMapperView)lView).updateActionState();
					}
				}
			}
		}
	...
	private boolean shouldProceed()
	{
		...
		if( ConcernMapper.getDefault().isDirty() )
		{
		    lReturn = MessageDialog.openQuestion( PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), 
		            ConcernMapper.getResourceString( "actions.LoadConcernModelAction.QuestionDialogTitle" ),
		            ConcernMapper.getResourceString( "actions.LoadConcernModelAction.WarningOverwrite"	));
		}
		return lReturn;
	}
	 ...
	 */
	static class LoadMonitor implements ca.mcgill.cs.serg.cm.model.io.IProgressMonitor
    {
