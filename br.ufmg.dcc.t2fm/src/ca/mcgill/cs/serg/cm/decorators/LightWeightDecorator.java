...
package ca.mcgill.cs.serg.cm.decorators;
...
public class LightWeightDecorator extends LabelProvider implements ConcernModelChangeListener, ILightweightLabelDecorator, IPropertyChangeListener
{
	...
	public void decorate( Object pElement, IDecoration pDecoration ) 
	{
		...
		String[] lConcernNames = ConcernMapper.getDefault().getConcernModel().getConcernNames();
		
		//For each concern, get the elements it contains
		for( String lConcernName : lConcernNames )
		{
			Set<Object> lCurrentConcernElements = ConcernMapper.getDefault().getConcernModel().getElements( lConcernName );
			...
			{
				int lDegree = ConcernMapper.getDefault().getConcernModel().getDegree( lConcernName, pElement );
				int lThreshold = ConcernMapper.getDefault().getPreferenceStore().getInt( ConcernMapperPreferencePage.P_FILTER_THRESHOLD );
				boolean lFilter = ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_FILTER_ENABLED );
		...
		//add the decorations
		if( lDoDecorateSuffix && ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SUFFIX_ENABLED ))
		{
			pDecoration.addSuffix( " "+lConcerns.toString() );
		}
		if( lDoDecorateFont && ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_BOLD_ENABLED ))
		{
	...
	private Set<Object> getAllParents( Set<Object> pElements, String pConcern)
	{
		...
		{	
			...
			{
				if ( ConcernMapper.getDefault().getConcernModel().getDegree( pConcern, lNext ) <
						ConcernMapper.getDefault().getPreferenceStore().getInt( ConcernMapperPreferencePage.P_FILTER_THRESHOLD ))
				{
		...
		if( !ConcernMapper.getDefault().getPreferenceStore().getString( ConcernMapperPreferencePage.P_DECORATION_LIMIT ).equals( "" ))
		{
			lTopParent = Integer.parseInt( ConcernMapper.getDefault().getPreferenceStore().
	...
	public static LightWeightDecorator getDecorator()
	{
		IDecoratorManager lDecoratorManager =
			ConcernMapper.getDefault().getWorkbench().getDecoratorManager();
