...
import ca.mcgill.cs.serg.cm.actions.ClearAction;
import ca.mcgill.cs.serg.cm.actions.CollapseAllAction;
import ca.mcgill.cs.serg.cm.actions.DeleteAction;
import ca.mcgill.cs.serg.cm.actions.FilterAction;
import ca.mcgill.cs.serg.cm.actions.LoadConcernModelAction;
import ca.mcgill.cs.serg.cm.actions.NewConcernAction;
import ca.mcgill.cs.serg.cm.actions.RenameConcernAction;
import ca.mcgill.cs.serg.cm.actions.SaveAction;
import ca.mcgill.cs.serg.cm.actions.SaveAsAction;
import ca.mcgill.cs.serg.cm.actions.ShowInconsistentElementsAction;
 ...
 * the ConcernMapper plug-in. 
 */
public class ConcernMapperView extends ViewPart implements ConcernModelChangeListener, IPropertyChangeListener
{
	...
	public void createPartControl( Composite pParent )
	{
		...
		if( !(ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_FILTER_ENABLED)))
		{
		...
		lGridData.verticalAlignment = GridData.FILL;
 		...
 		initSliderPanel( pParent, ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_SLIDER ));
 		initCommentBox( pParent, ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_COMMENTS ) );
		...
		ConcernMapper.getDefault().getPreferenceStore().addPropertyChangeListener( this );
	...
	private void updateVisibilityStatus()
	{
		...
		boolean lVisible = ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_SLIDER );
		...
		lVisible = ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_COMMENTS );
					...
					lDisplay.asyncExec( new Runnable()
						...
						public void run()
						{
							...
							else
							{
								...
								aViewer.setInput( ConcernMapper.getDefault().getConcernModel() );
	...
	private void updateFilters()
	{
		...
		if(ConcernMapper.getDefault().getPreferenceStore().getBoolean(
	...
	{
		aSaveAction.setEnabled( ConcernMapper.getDefault().isDirty() );
	...
	private void hookContextMenu() 
	{
		...
		lMenuManager.addMenuListener( new IMenuListener() 
			...
			{
				ConcernMapperView.this.fillContextMenu( pManager );
			}
				});
				...
				if( lSelection instanceof IStructuredSelection )
				{
					...
					for( int lI = 0; lI<lStructuredSelectionArray.length; lI++ )
					{
						...
						if( lElement instanceof WrapperNode )
						{
							...
							{
								ConcernMapper.getDefault().getConcernModel().
				...
				if( lSelection instanceof IStructuredSelection )
				{
					...
					if( lStructuredSelection.size() == 1 )
					{
						...
						if( lElement instanceof ConcernNode )
						{
							...
							if( ConcernMapper.getDefault().getConcernModel().exists( lNode.getConcern() ))
							{
								if( !ConcernMapper.getDefault().getConcernModel().getConcernComment(
										lNode.getConcern()).equals( aCommentBox.getText()))
								{
									ConcernMapper.getDefault().getConcernModel().setConcernComment( lNode.getConcern(), aCommentBox.getText());
								}
							}
						}
						else if( lElement instanceof WrapperNode )
						{
							...
							if( ConcernMapper.getDefault().getConcernModel().exists( lNode.getConcern(), lNode.getElement() ))
							{
								if( !ConcernMapper.getDefault().getConcernModel().getElementComment(
										lNode.getConcern(), lNode.getElement()).equals( aCommentBox.getText()))
								{
									ConcernMapper.getDefault().getConcernModel().setElementComment(
	...
	public void dispose() 
	{
		...
		ConcernMapper.getDefault().getPreferenceStore().removePropertyChangeListener( this );
    ...
    class ConcernMapperViewDropAdapter extends ViewerDropAdapter
    {
		...
		public boolean performDrop( Object pData ) 
        {
            ...
            if( getCurrentTarget() != null )
            {
            	...
            	assert ConcernMapper.getDefault().getConcernModel().exists( lTarget );
                Object lSelection = LocalSelectionTransfer.getInstance().getSelection();
                if( lSelection instanceof IStructuredSelection )
                {
                	...
                	try
                	{
                		for( Iterator lI = ((IStructuredSelection)lSelection).iterator(); lI.hasNext(); )
                		{
                			...
                			if( supportedType( lElement ) )
                			{
        						...
        						{
        							if( !ConcernMapper.getDefault().getConcernModel().exists( lTarget, lField ))
                        			{
                        				if( lNext instanceof WrapperNode ) // we're dealing with within-model dnd
                        				{
                        					int lDegree = ConcernMapper.getDefault().getConcernModel().
                        						getDegree( ((WrapperNode)lNext).getConcern(), lField );
                        					deleteExistingElement( (WrapperNode)lNext, lField);
                        					ConcernMapper.getDefault().getConcernModel().addElement( lTarget, lField, lDegree );
                        					lReturn = true;
                        				}
                        				else 
                        				{
                        					ConcernMapper.getDefault().getConcernModel().addElement( lTarget, lField, MAX_DEGREE );
                        					lReturn = true;
                        				} 
                        			}
        						}
        						...
        						{
        							if( !ConcernMapper.getDefault().getConcernModel().exists( lTarget, lMethod ))
                        			{
                        				if( lNext instanceof WrapperNode ) // we're dealing with within-model dnd
                        				{
                        					int lDegree = ConcernMapper.getDefault().getConcernModel().
                        					...
                        					ConcernMapper.getDefault().getConcernModel().addElement( lTarget, lMethod, lDegree );
                        					lReturn = true;
                        				}
                        				else 
                        				{
                        					ConcernMapper.getDefault().getConcernModel().addElement( lTarget, lMethod, MAX_DEGREE );
                        					lReturn = true;
                        				} 
                        			}
        						}
                			...
                			{
                				if( !ConcernMapper.getDefault().getConcernModel().exists( lTarget, lElement ))
                				{
                					if( lNext instanceof WrapperNode ) // we're dealing with within-model dnd
                					{
                						int lDegree = ConcernMapper.getDefault().getConcernModel().
                						...
                						ConcernMapper.getDefault().getConcernModel().addElement( lTarget, lElement, lDegree );
                						lReturn = true;
                					}
                					else 
                					{
                						ConcernMapper.getDefault().getConcernModel().addElement( lTarget, lElement, MAX_DEGREE );
                						lReturn = true;
                					} 
                				}
		...
		private boolean supportedType( IJavaElement pElement )
		{
      	...
      	private void deleteExistingElement( WrapperNode pNode, IJavaElement pElement  )
      	{
      		if (!aCopy)
      		{
      			ConcernMapper.getDefault().getConcernModel().deleteElement( pNode.getConcern(), pElement );
      		}
      	}
    ...
    class ConcernMapperSelectionChangedListener implements ISelectionChangedListener
    {
		...
		{	
			...
			if( lSelection instanceof IStructuredSelection )
			{
				...
				if( lStructuredSelection.size() == 1 )
				{
					...
					if( lElement instanceof ConcernNode )
					{
						getViewSite().getActionBars().getStatusLineManager().setMessage( ((ConcernNode)lElement).getConcernName() + " (" + 
								ConcernMapper.getDefault().getConcernModel().getAllElements( ((ConcernNode)lElement).getConcernName()).size() + 
								" elements)");
						ConcernNode lNode = (ConcernNode)lElement;
						aCommentBox.setEnabled( true );
						aCommentBox.setVisible( true );
						aCommentBox.removeModifyListener( aModifyListener );
						aCommentBox.setText( ConcernMapper.getDefault().getConcernModel().getConcernComment( lNode.getConcern() ));
						aCommentBox.addModifyListener( aModifyListener );
					}
					if( lElement instanceof WrapperNode )
					{
						...
						if( ConcernMapper.getDefault().getConcernModel().exists( lNode.getConcern(), lNode.getElement() ))
						{
							...
							aCommentBox.setEnabled( true );
							aCommentBox.setVisible( true );
							int lDegree = ConcernMapper.getDefault().getConcernModel().getDegree( lNode.getConcern(), lNode.getElement() );
							aDegreeSlider.setSelection( lDegree );
							aDegreeValue.setText( new Integer( lDegree ).toString() );
							aCommentBox.setText(
									ConcernMapper.getDefault().getConcernModel().getElementComment( lNode.getConcern(), lNode.getElement() ) );
						}
				...
				{
					getViewSite().getActionBars().getStatusLineManager().setMessage(
							lStructuredSelection.size() + " " + ConcernMapper.getResourceString( "views.ConcernMapperView.itemsSelected"));
					aDegreeSlider.setEnabled( true );
