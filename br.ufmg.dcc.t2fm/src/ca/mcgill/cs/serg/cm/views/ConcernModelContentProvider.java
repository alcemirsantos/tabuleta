 ...
 * they declare.
 */
class ConcernModelContentProvider implements IStructuredContentProvider, ITreeContentProvider 
{
    	...
    	for( String lConcernName : aConcernModel.getConcernNames() )
    	{
    		...
    		for( Object lConcernElement : aConcernModel.getAllElements( lConcernName ))
    		{
    			if( lConcernElement instanceof IJavaElement )
    			{
    				...
    				{
    					if( ConcernMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_INCONSISTENT_ELEMENTS ))
    					{
    						addToMap( lMap, lJavaElement, lConcernNode );
    					}
    				}
