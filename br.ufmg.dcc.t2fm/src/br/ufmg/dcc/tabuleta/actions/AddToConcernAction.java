...
public class AddToConcernAction implements IObjectActionDelegate, IEditorActionDelegate, IMenuCreator, ConcernModelChangeListener
{
	...
	private void fillMenu()
	{
		...
		String[] lConcerns = Tabuleta.getDefault().getConcernModel().getConcernNames();
		...
		if( !Tabuleta.getDefault().getConcernModel().exists( DEFAULT_CONCERN_NAME ) )
		{
		...
		else
		{
			...
			{
				if( !Tabuleta.getDefault().getConcernModel().exists( DEFAULT_CONCERN_NAME + " " + lIndex ) )
				{
	...
	private class AddToConcernMenuItemListener extends SelectionAdapter
	{
		public void widgetSelected( SelectionEvent pEvent )
		{
			...
			if( lSelection != null )
			{
				if( lSelection.isEmpty() )
				{
					...
					{
						lEditorStatusLine.setMessage( true, Tabuleta.getResourceString( 
				...
				for( Iterator<IJavaElement> lJavaElementIterator = lSelection.iterator(); lJavaElementIterator.hasNext(); )
				{
					...
					else
					{
						...
						String lMessage = Tabuleta.getResourceString( "actions.AddToConcernAction.ElementNotSupported" );
						if( lEditorStatusLine != null )
						{
		...
		private boolean supportedType( IJavaElement pElement )
		{
      	...
      	{
      		if( !Tabuleta.getDefault().getConcernModel().exists( pConcern ) )
			{
				Tabuleta.getDefault().getConcernModel().newConcern( pConcern );
			}
			if( !Tabuleta.getDefault().getConcernModel().exists( pConcern, pElement ) )
			{
				Tabuleta.getDefault().getConcernModel().addElement( pConcern, pElement, MAX_DEGREE );
				T2FMappingView lView = (T2FMappingView)PlatformUI.getWorkbench().
					getActiveWorkbenchWindow().getActivePage().findView( Tabuleta.ID_VIEW );
