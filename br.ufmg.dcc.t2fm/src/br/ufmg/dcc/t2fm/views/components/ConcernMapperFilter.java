...
public class ConcernMapperFilter extends ViewerFilter
{
	...
	public boolean select( Viewer pViewer, Object pParentElement, Object pElement )
	{
		...
		if( pElement instanceof WrapperNode )
		{
			...
			{
				if( Test2FeatureMapper.getDefault().getConcernModel().getDegree( lNode.getConcern(), lNode.getElement() ) > lThreshold )
				{
					lReturn = true;
				}
			}
			...
			for( IConcernMapperViewNode lNext : lDescendants )
			{
				...
				if( Test2FeatureMapper.getDefault().getConcernModel().exists( lNext.getConcern(), ((WrapperNode)lNext).getElement() ))
				{
					if(Test2FeatureMapper.getDefault().getConcernModel().getDegree( lNext.getConcern(), ((WrapperNode)lNext).getElement()) > lThreshold )
					{
						lReturn = true;	
					}
				}
