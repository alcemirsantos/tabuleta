...
import br.ufmg.dcc.t2fm.views.components.ConcernMapperFilter;
import br.ufmg.dcc.t2fm.views.components.ConcernModelContentProvider;
import br.ufmg.dcc.t2fm.views.components.ConcernModelLabelProvider;
import br.ufmg.dcc.t2fm.views.components.ConcernNode;
import br.ufmg.dcc.t2fm.views.components.IConcernMapperViewNode;
...
public class MapView extends ViewPart implements ConcernModelChangeListener, IPropertyChangeListener {

	/**
	 * The ID of the view as specified by the extension.
	 */
	public static final String ID = "br.ufmg.dcc.t2fm.views.MapView";
		...
		public void createPartControl( Composite pParent )
		{
			...
			lGridData.verticalAlignment = GridData.FILL;
	 		...
	 		initSliderPanel( pParent, Test2FeatureMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_SLIDER ));
	 		initCommentBox( pParent, Test2FeatureMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_COMMENTS ) );
		...
		private void updateVisibilityStatus()
		{
			...
			boolean lVisible = Test2FeatureMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_SLIDER );
			...
			lVisible = Test2FeatureMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_SHOW_COMMENTS );
						...
						lDisplay.asyncExec( new Runnable()
							...
							public void run()
							{
								...
								else
								{
									...
									aViewer.setInput( Test2FeatureMapper.getDefault().getConcernModel() );
		...
		private void updateFilters()
		{
			...
			if(Test2FeatureMapper.getDefault().getPreferenceStore().getBoolean(
		...
		private void hookContextMenu() 
		{
			...
			lMenuManager.addMenuListener( new IMenuListener() 
				...
				{
					MapView.this.fillContextMenu( pManager );
				}
					});
					...
					if( lSelection instanceof IStructuredSelection )
					{
						...
						for( int lI = 0; lI<lStructuredSelectionArray.length; lI++ )
						{
							...
							if( lElement instanceof WrapperNode )
							{
								...
								if( Test2FeatureMapper.getDefault().getConcernModel().exists( lNode.getConcern(), lNode.getElement() ))
								{
									Test2FeatureMapper.getDefault().getConcernModel().
					...
					if( lSelection instanceof IStructuredSelection )
					{
						...
						if( lStructuredSelection.size() == 1 )
						{
							...
							if( lElement instanceof ConcernNode )
							{
								...
								if( Test2FeatureMapper.getDefault().getConcernModel().exists( lNode.getConcern() ))
								{
									if( !Test2FeatureMapper.getDefault().getConcernModel().getConcernComment(
											lNode.getConcern()).equals( aCommentBox.getText()))
									{
										Test2FeatureMapper.getDefault().getConcernModel().setConcernComment( lNode.getConcern(), aCommentBox.getText());
									}
								}
							}
							else if( lElement instanceof WrapperNode )
							{
								...
								if( Test2FeatureMapper.getDefault().getConcernModel().exists( lNode.getConcern(), lNode.getElement() ))
								{
									if( !Test2FeatureMapper.getDefault().getConcernModel().getElementComment(
											lNode.getConcern(), lNode.getElement()).equals( aCommentBox.getText()))
									{
										Test2FeatureMapper.getDefault().getConcernModel().setElementComment(
...
//						pManager.add(new CoverageAsAction());
		...
		public void dispose() 
		{
			...
			Test2FeatureMapper.getDefault().getPreferenceStore().removePropertyChangeListener( this );
	    ...
	    class ConcernMapperViewDropAdapter extends ViewerDropAdapter
	    {
			...
			public boolean performDrop( Object pData ) 
	        {
	            ...
	            if( getCurrentTarget() != null )
	            {
	            	...
	            	assert Test2FeatureMapper.getDefault().getConcernModel().exists( lTarget );
	                Object lSelection = LocalSelectionTransfer.getInstance().getSelection();
	                if( lSelection instanceof IStructuredSelection )
	                {
	                	...
	                	try
	                	{
	                		for( Iterator lI = ((IStructuredSelection)lSelection).iterator(); lI.hasNext(); )
	                		{
	                			...
	                			if( supportedType( lElement ) )
	                			{
	        						...
	        						{
	        							if( !Test2FeatureMapper.getDefault().getConcernModel().exists( lTarget, lField ))
	                        			{
	                        				if( lNext instanceof WrapperNode ) // we're dealing with within-model dnd
	                        				{
	                        					int lDegree = Test2FeatureMapper.getDefault().getConcernModel().
	                        						getDegree( ((WrapperNode)lNext).getConcern(), lField );
	                        					deleteExistingElement( (WrapperNode)lNext, lField);
	                        					Test2FeatureMapper.getDefault().getConcernModel().addElement( lTarget, lField, lDegree );
	                        					lReturn = true;
	                        				}
	                        				else 
	                        				{
	                        					Test2FeatureMapper.getDefault().getConcernModel().addElement( lTarget, lField, MAX_DEGREE );
	                        					lReturn = true;
	                        				} 
	                        			}
	        						}
	        						...
	        						{
	        							if( !Test2FeatureMapper.getDefault().getConcernModel().exists( lTarget, lMethod ))
	                        			{
	                        				if( lNext instanceof WrapperNode ) // we're dealing with within-model dnd
	                        				{
	                        					int lDegree = Test2FeatureMapper.getDefault().getConcernModel().
	                        					...
	                        					Test2FeatureMapper.getDefault().getConcernModel().addElement( lTarget, lMethod, lDegree );
	                        					lReturn = true;
	                        				}
	                        				else 
	                        				{
	                        					Test2FeatureMapper.getDefault().getConcernModel().addElement( lTarget, lMethod, MAX_DEGREE );
	                        					lReturn = true;
	                        				} 
	                        			}
	        						}
	                			...
	                			{
	                				if( !Test2FeatureMapper.getDefault().getConcernModel().exists( lTarget, lElement ))
	                				{
	                					if( lNext instanceof WrapperNode ) // we're dealing with within-model dnd
	                					{
	                						int lDegree = Test2FeatureMapper.getDefault().getConcernModel().
	                						...
	                						Test2FeatureMapper.getDefault().getConcernModel().addElement( lTarget, lElement, lDegree );
	                						lReturn = true;
	                					}
	                					else 
	                					{
	                						Test2FeatureMapper.getDefault().getConcernModel().addElement( lTarget, lElement, MAX_DEGREE );
	                						lReturn = true;
	                					} 
	                				}
			...
			private boolean supportedType( IJavaElement pElement )
			{
	      	...
	      	private void deleteExistingElement( WrapperNode pNode, IJavaElement pElement  )
	      	{
	      		if (!aCopy)
	      		{
	      			Test2FeatureMapper.getDefault().getConcernModel().deleteElement( pNode.getConcern(), pElement );
	      		}
	      	}
	    ...
	    class ConcernMapperSelectionChangedListener implements ISelectionChangedListener
	    {
			...
			{	
				...
				if( lSelection instanceof IStructuredSelection )
				{
					...
					if( lStructuredSelection.size() == 1 )
					{
						...
						if( lElement instanceof ConcernNode )
						{
							getViewSite().getActionBars().getStatusLineManager().setMessage( ((ConcernNode)lElement).getConcernName() + " (" + 
									Test2FeatureMapper.getDefault().getConcernModel().getAllElements( ((ConcernNode)lElement).getConcernName()).size() + 
									" elements)");
							ConcernNode lNode = (ConcernNode)lElement;
							aCommentBox.setEnabled( true );
							aCommentBox.setVisible( true );
							aCommentBox.removeModifyListener( aModifyListener );
							aCommentBox.setText( Test2FeatureMapper.getDefault().getConcernModel().getConcernComment( lNode.getConcern() ));
							aCommentBox.addModifyListener( aModifyListener );
						}
						if( lElement instanceof WrapperNode )
						{
							...
							if( Test2FeatureMapper.getDefault().getConcernModel().exists( lNode.getConcern(), lNode.getElement() ))
							{
								...
								aCommentBox.setEnabled( true );
								aCommentBox.setVisible( true );
								int lDegree = Test2FeatureMapper.getDefault().getConcernModel().getDegree( lNode.getConcern(), lNode.getElement() );
								aDegreeSlider.setSelection( lDegree );
								aDegreeValue.setText( new Integer( lDegree ).toString() );
								aCommentBox.setText(
										Test2FeatureMapper.getDefault().getConcernModel().getElementComment( lNode.getConcern(), lNode.getElement() ) );
							}
					...
					{
						getViewSite().getActionBars().getStatusLineManager().setMessage(
								lStructuredSelection.size() + " " + Test2FeatureMapper.getResourceString( "views.MapView.itemsSelected"));
						aDegreeSlider.setEnabled( true );
