...
public class FilterDialog extends MessageDialogWithToggle 
{
	...
	{
		super( pParentShell, 
			   Test2FeatureMapper.getResourceString( "views.ConcernMapperView.FilterDialogTitle" ), 
			   null, 
			   ...
			   Test2FeatureMapper.getDefault().getPreferenceStore().getBoolean( ConcernMapperPreferencePage.P_FILTER_ENABLED ));
	}
	...
	protected void buttonPressed( int pButtonID ) 
	{
		if( pButtonID == 0 )
		{
			Test2FeatureMapper.getDefault().getPreferenceStore().
	...
	protected Control createCustomArea( Composite pParent )
	{
		...
		aThresholdSlider.setSelection(
				Test2FeatureMapper.getDefault().getPreferenceStore().getInt(
