...
public final class ProblemManager
{
	...
	public static void reportException( Exception pException )
	{
		...
		lStatuses[0] = new Status( IStatus.ERROR, Test2FeatureMapper.ID_PLUGIN, IStatus.OK, pException.getClass().getName(), pException );
		for( int lI = 0 ; lI < lStackElements.length; lI++ )
		{
			lStatuses[lI+1] = new Status( IStatus.ERROR, Test2FeatureMapper.ID_PLUGIN, IStatus.OK, "     " + lStackElements[lI].toString(), pException );
		}
		
		String lMessage = Test2FeatureMapper.getResourceString( "ui.ProblemManager.DefaultMessage" );
		String lTitle = Test2FeatureMapper.getResourceString( "ui.ProblemManager.DialogTitle" );
		if( pException.getMessage() != null )
		{
			lMessage = pException.getMessage();
		}
		MultiStatus lStatus = new MultiStatus( Test2FeatureMapper.ID_PLUGIN, IStatus.OK, lStatuses, lMessage, pException );
		ErrorDialog.openError( PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), lTitle, lMessage, lStatus, IStatus.ERROR );
		Test2FeatureMapper.getDefault().getLog().log( lStatus );
	}
	...
	public static void reportExceptionMessage( Exception pException )
	{
		...
		String lMessage = Test2FeatureMapper.getResourceString( "ui.ProblemManager.DefaultMessage" );
		final String lTitle = Test2FeatureMapper.getResourceString( "ui.ProblemManager.DialogTitle" );
		if( pException.getMessage() != null )
		{
			lMessage = pException.getMessage();
		}
		final String lMessage2 = lMessage; 
		MultiStatus lStatus = new MultiStatus( Test2FeatureMapper.ID_PLUGIN, IStatus.OK, lStatuses, lMessage2, pException );
